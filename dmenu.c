...
/* enums */
enum { SchemeNorm, SchemeSel, SchemeOut, SchemeNormHighlight, SchemeSelHighlight, SchemeHp, SchemeOutHighlight, SchemeLast }; /* color schemes */
struct item {
	char *text;
...
readxresources(void) {
	...
	if ((xrm = XResourceManagerString(drw->dpy))) {
		...
		if (XrmGetResource(xdb, "dmenu.normbg", "*", &type, &xval))
			colors[SchemeNorm][ColBg] = strdup(xval.addr);
		else
			colors[SchemeNorm][ColBg] = strdup(colors[SchemeNorm][ColBg]);
		if (XrmGetResource(xdb, "dmenu.normfg", "*", &type, &xval))
			colors[SchemeNorm][ColFg] = strdup(xval.addr);
		else
			colors[SchemeNorm][ColFg] = strdup(colors[SchemeNorm][ColFg]);
		if (XrmGetResource(xdb, "dmenu.selbg", "*", &type, &xval))
			colors[SchemeSel][ColBg] = strdup(xval.addr);
		else
			colors[SchemeSel][ColBg] = strdup(colors[SchemeSel][ColBg]);
		if (XrmGetResource(xdb, "dmenu.selfg", "*", &type, &xval))
			colors[SchemeSel][ColFg] = strdup(xval.addr);
		else
			colors[SchemeSel][ColFg] = strdup(colors[SchemeSel][ColFg]);
		if (XrmGetResource(xdb, "dmenu.selhlfg", "*", &type, &xval))
			colors[SchemeSelHighlight][ColFg] = strdup(xval.addr);
		else
			colors[SchemeSelHighlight][ColFg] = strdup(colors[SchemeSelHighlight][ColFg]);
		if (XrmGetResource(xdb, "dmenu.selhlbg", "*", &type, &xval))
			colors[SchemeSelHighlight][ColBg] = strdup(xval.addr);
		else
			colors[SchemeSelHighlight][ColBg] = strdup(colors[SchemeSelHighlight][ColBg]);
		if (XrmGetResource(xdb, "dmenu.normhlfg", "*", &type, &xval))
			colors[SchemeNormHighlight][ColFg] = strdup(xval.addr);
		else
			colors[SchemeNormHighlight][ColFg] = strdup(colors[SchemeNormHighlight][ColFg]);
		if (XrmGetResource(xdb, "dmenu.normhlbg", "*", &type, &xval))
...
main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
		...
		/* these options take one argument */
			...
			colortemp[7] = argv[++i];      /* not sure */
		else if (!strcmp(argv[i], "-hf"))  /* low priority background color */
			colortemp[6] = argv[++i];	   /* not sure */
	...
	if ( colortemp[4])
	   colors[SchemeSelHighlight][ColFg]   = strdup(colortemp[4]);
	if ( colortemp[5])
	   colors[SchemeSelHighlight][ColBg]   = strdup(colortemp[5]);
	if ( colortemp[6])
	   colors[SchemeNormHighlight][ColFg]   = strdup(colortemp[6]);
	if ( colortemp[7])
	   colors[SchemeNormHighlight][ColBg]   = strdup(colortemp[7]);
